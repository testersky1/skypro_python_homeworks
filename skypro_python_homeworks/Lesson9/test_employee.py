from employee_api import ApiEmployee
from company import Company
from emloyee_db import DbEmployee
...
def test_create_employee():
    db_result = db.get_list_employee(company_id)
    db.add_new_employee("Данил", "Тарханов", "88005553535", True, company_id)
    data_employee = api.get_new_employee2(db.get_id_new_employee())
    data_employee = data_employee.json()
    assert data_employee["firstName"] == "Данил"
    assert data_employee["lastName"] == "Тарханов"
    assert data_employee["phone"] == "88005553535"
...
def test_edit_employee():
    db.add_new_employee("Данил", "Тарханов", "88005553535", True, company_id)
    id = db.get_id_new_employee()
    db.edit_employee("Данил", "Бильярдов", "88888888888", True, company_id, id)
    data_employee = api.get_new_employee2(id)
    data_employee = data_employee.json()
    assert data_employee["firstName"] == "Данил"
    assert data_employee["lastName"] == "Бильярдов"
    assert data_employee["phone"] == "88888888888"
    assert data_employee["isActive"] == True
    assert data_employee["companyId"] == company_id
    id = db.get_id_new_employee()
    db.delete(id)